...
(ert-deftest multi-cursor-mark-set-mark-sets-every-cursor-mark ()
  "C-SPC must set each cursor's own mark at its own point."
  (with-temp-buffer
    (insert "alpha\nbeta\ngamma\n")
    (goto-char 1)
    (multi-cursor-add-at-point 7)
    ...
    (should (equal (mapcar (lambda (s) (list (plist-get s :point)
                                             (plist-get s :mark)
                                             (plist-get s :mark-active)))
                           (multi-cursor-selections))
                   '((7 7 t) (12 12 t))))))

(ert-deftest multi-cursor-mark-selections-extend-on-movement ()
  "After setting the mark, ordinary motion must extend every selection."
  (with-temp-buffer
    (insert "alpha\nbeta\ngamma\n")
    (goto-char 1)
    (multi-cursor-add-at-point 7)
    ...
    (let ((selection (car (multi-cursor-selections))))
      (should (= (plist-get selection :point) 8))
      (should (= (plist-get selection :mark) 7))
      (should (plist-get selection :mark-active)))))

(ert-deftest multi-cursor-mark-repeat-toggles-activation ()
  "Repeating C-SPC must toggle activation without moving any mark."
  (with-temp-buffer
    (insert "alpha\nbeta\ngamma\n")
    (goto-char 1)
    (multi-cursor-add-at-point 7)
...
(ert-deftest multi-cursor-mark-prefixed-set-mark-is-rejected ()
  ...
  (with-temp-buffer
    ...
    (let ((transient-mark-mode t)
          (prefix-arg '(4))
          (last-command nil))
      (should-error (command-execute 'set-mark-command) :type 'user-error)
      (should-not (region-active-p)))
    (should-not (plist-get (car (multi-cursor-selections)) :mark-active))))

(ert-deftest multi-cursor-mark-exchange-swaps-every-cursor ()
  "C-x C-x must swap point and mark independently at every cursor."
  (with-temp-buffer
    (insert "alpha\nbeta\ngamma\n")
    (goto-char 1)
    (multi-cursor-add-at-point 7)
    (let ((transient-mark-mode t))
      ...
      (should (= (point) 3))
      (should (= (mark t) 1))
      (command-execute 'exchange-point-and-mark)
      (should (= (point) 1))
      (should (= (mark t) 3))
      (should (region-active-p)))
    (let ((selection (car (multi-cursor-selections))))
      (should (= (plist-get selection :point) 7))
      (should (= (plist-get selection :mark) 9))
      (should (plist-get selection :mark-active)))))

(ert-deftest multi-cursor-mark-exchange-without-mark-is-atomic ()
  "A cursor without a mark must reject the whole command unchanged."
  (with-temp-buffer
    (insert "alpha\nbeta\ngamma\n")
    (goto-char 1)
    (push-mark 3 t t)
    (multi-cursor-add-at-point 7)
    (let ((transient-mark-mode t))
      (should-error (command-execute 'exchange-point-and-mark)
                    :type 'user-error))
    (should (= (point) 1))
    (should (= (mark t) 3))
    (let ((selection (car (multi-cursor-selections))))
