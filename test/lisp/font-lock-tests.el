...
(declare-function font-lock--async-fontify-region "font-lock"
                  (beg end keywords &optional normalized))
...
(ert-deftest font-lock-commit-queue-rejects-late-stale-buffer-work ()
  "Late stale work does not replace current queued commits."
  (let ((old-queue font-lock--commit-queue)
        (old-tail font-lock--commit-queue-tail)
        (old-timer font-lock--commit-timer)
        seen)
...
(ert-deftest font-lock-commit-queue-drops-stale-pending-redisplay ()
  "Deferred redisplay requests are dropped after the source buffer changes."
  (let ((font-lock-commit-dispatch-budget nil)
        (font-lock-commit-defer-on-input t)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer)
        ...
        (with-temp-buffer
          ...
          (let ((buffer (current-buffer))
            ...
            (cl-letf (((symbol-function 'input-pending-p)
              ...
              (should (equal (font-lock--dispatch-commits)
                             '(:processed 1 :dropped 0 :remaining 0)))
              (should font-lock--pending-redisplay-requests)
              (insert "d")
              (setq input-pending nil)
              (should (equal (font-lock--dispatch-commits)
                             '(:processed 0 :dropped 0 :remaining 0)))
        ...
        (progn
          ...
          (cl-letf (((symbol-function 'input-pending-p)
            ...
            (pcase-let ((`((,flushed-buffer 1 3)) calls)
                        (`((,pending-buffer ,pending-tick 1 . 3))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-subexp-keywords ()
  "The async keyword path accepts simple dotted subexp specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          (insert "alpha beta")
          (let ((font-lock-async-keywords t)
                (font-lock-keywords '(("alpha \\(beta\\)" . 1)))
                (font-lock-keywords-only t)
                (font-lock-keywords-case-fold-search nil)
                (font-lock-set-defaults t)
                (font-lock-syntax-table nil)
            ...
            (font-lock-default-fontify-region (point-min) (point-max) nil)
            (should-not (get-text-property 7 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for subexp async font-lock"))
              (while (not (get-text-property 7 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-string-keywords ()
  "The async keyword path accepts bare string specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
            ...
            (font-lock-default-fontify-region (point-min) (point-max) nil)
            (should-not (get-text-property 1 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for string async font-lock"))
              (while (not (get-text-property 1 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-multi-highlight-keywords ()
  "The async keyword path accepts simple specs with multiple highlights."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
            ...
            (should-not (get-text-property 1 'face))
            (should-not (get-text-property 7 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for multi-highlight async font-lock"))
              (while (not (get-text-property 7 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-anchored-keywords ()
  "The async keyword path accepts simple anchored regexp specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
            ...
            (with-timeout (3 (ert-fail "Timed out waiting for anchored async font-lock"))
              (while (not (get-text-property 8 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
            (should (eq (get-text-property 1 'face)
                        'font-lock-keyword-face))
            (should (eq (get-text-property 8 'face)
                        'font-lock-variable-name-face))
            (should (eq (get-text-property 13 'face)
                        'font-lock-variable-name-face))
            (should-not (get-text-property 18 'face))))
      (when (timerp font-lock--commit-timer)
        (cancel-timer font-lock--commit-timer))
      (when font-lock--async-worker-pool
        (font-lock--async-shutdown-workers))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-anchored-goto-char ()
  "The async keyword path accepts safe anchored point movement."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
          ...
          (let ((font-lock-async-keywords t)
                (font-lock-keywords
                 '(("\\(anchor\\): item"
                    ...
                    ("item" (goto-char (match-beginning 0)) nil
                     (0 font-lock-variable-name-face)))))
                (font-lock-keywords-only t)
                (font-lock-keywords-case-fold-search nil)
                (font-lock-set-defaults t)
            ...
            (with-timeout (3 (ert-fail "Timed out waiting for anchored goto-char async font-lock"))
              (while (not (get-text-property 9 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
            (should (eq (get-text-property 1 'face)
...
(ert-deftest font-lock-default-fontify-region-can-use-async-anchored-goto-char-or ()
  "The async keyword path accepts safe alternative point movement."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
        ...
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
                (font-lock-keywords
                 '(("\\(global\\) \\(name\\)"
                    (1 font-lock-keyword-face)
                    ("name" (goto-char (or (match-beginning 2)
                                           (match-end 1)))
                     nil
                     (0 font-lock-variable-name-face)))))
                (font-lock-keywords-only t)
            ...
            (should-not (get-text-property 1 'face))
            (should-not (get-text-property 8 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for anchored or-goto async font-lock"))
              (while (not (get-text-property 8 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-quoted-face-keywords ()
  "The async keyword path accepts simple dotted quoted face specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
            ...
            (font-lock-default-fontify-region (point-min) (point-max) nil)
            (should-not (get-text-property 1 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for quoted-face async font-lock"))
              (while (not (get-text-property 1 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-property-list-faces ()
  "The async keyword path accepts simple literal property-list faces."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
                (font-lock-keywords
                 '(("alpha" 0
                    '(face font-lock-keyword-face help-echo "async alpha")
                    t)))
                (font-lock-keywords-only t)
                (font-lock-keywords-case-fold-search nil)
                (font-lock-set-defaults t)
            ...
            (with-timeout (3 (ert-fail "Timed out waiting for property-list async font-lock"))
              (while (not (get-text-property 1 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-anonymous-faces ()
  "The async keyword path accepts literal anonymous face specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
                (font-lock-keywords
                 '(("alpha" 0
                    '(face (:weight bold) help-echo "async anonymous")
                    t)))
                (font-lock-keywords-only t)
                (font-lock-keywords-case-fold-search nil)
                (font-lock-set-defaults t)
            ...
            (should-not (get-text-property 1 'face))
            (should-not (get-text-property 1 'help-echo))
            (with-timeout (3 (ert-fail "Timed out waiting for anonymous-face async font-lock"))
              (while (not (get-text-property 1 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-can-use-async-eval-keywords ()
  "The async keyword path accepts simple evaluated keyword specs."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
                (font-lock-keywords
                 '((eval . (cons "alpha" 'font-lock-keyword-face))))
                (font-lock-keywords-only t)
                (font-lock-keywords-case-fold-search nil)
                (font-lock-set-defaults t)
                (font-lock-syntax-table nil)
            ...
            (font-lock-default-fontify-region (point-min) (point-max) nil)
            (should-not (get-text-property 1 'face))
            (with-timeout (3 (ert-fail "Timed out waiting for eval async font-lock"))
              (while (not (get-text-property 1 'face))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
...
(ert-deftest font-lock-default-fontify-region-async-respects-ignored-keywords ()
  "The async keyword path respects `font-lock-ignore'."
  (let ((old-pool font-lock--async-worker-pool)
        (old-queue font-lock--commit-queue)
        (old-timer font-lock--commit-timer))
    (setq font-lock--async-worker-pool nil
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (let ((font-lock-async-keywords t)
                ...
                (font-lock-extend-region-functions nil))
            (font-lock-default-fontify-region (point-min) (point-max) nil)
            (with-timeout (3 (ert-fail "Timed out waiting for ignored async font-lock"))
              (while (or font-lock--commit-queue
                         (and font-lock--async-worker-pool
                              (not (font-lock-tests--async-workers-idle-p))))
                (accept-process-output nil 0.01)
                (when font-lock--commit-queue
                  (font-lock--dispatch-commits))))
            (should-not (get-text-property 1 'face))
