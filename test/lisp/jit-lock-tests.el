...
(ert-deftest jit-lock-fontify-now-yields-between-chunks-on-input ()
  (ert-with-test-buffer (:name "xxx")
    (insert "aa bb cc")
    (with-silent-modifications
      (put-text-property 1 3 'fontified nil)
      (put-text-property 4 6 'fontified t)
      (put-text-property 7 9 'fontified nil))
      ...
      (cl-letf (((symbol-function 'input-pending-p)
        ...
        (unwind-protect
            (let ((jit-lock-functions
              ...
              (jit-lock-fontify-now (point-min) (point-max))
              (should (equal (nreverse calls) '((1 . 4))))
              (should (eq (get-text-property 1 'fontified) t))
              (should (eq (get-text-property 6 'fontified) 'defer))
...
(ert-deftest jit-lock-deferred-fontify-yields-while-input-is-pending ()
  (ert-with-test-buffer (:name "xxx")
    ...
    (let ((jit-lock-defer-on-input t)
          (jit-lock-deferred-scan-check-interval 1)
...
(ert-deftest jit-lock-deferred-fontify-respects-scan-budget ()
  (ert-with-test-buffer (:name "xxx")
    (insert "aa bb cc")
    (with-silent-modifications
      (put-text-property 1 3 'fontified 'defer)
      ...
      (put-text-property 7 9 'fontified 'defer))
    (let ((jit-lock-defer-on-input t)
          (jit-lock-deferred-scan-budget 0.001)
          (jit-lock-deferred-scan-check-interval 1)
          (jit-lock-defer-timer (timer-create))
          (jit-lock--defer-timer-input-only t)
          (jit-lock-defer-buffers (list (current-buffer)))
...
(ert-deftest jit-lock-deferred-fontify-checks-yield-in-batches ()
  (ert-with-test-buffer (:name "xxx")
    (insert "aa bb cc dd")
    (with-silent-modifications
      (put-text-property 1 3 'fontified 'defer)
      (put-text-property 4 6 'fontified 'defer)
      (put-text-property 7 9 'fontified 'defer)
      (put-text-property 10 12 'fontified 'defer))
    (let ((jit-lock-defer-on-input t)
          (jit-lock-deferred-scan-check-interval 3)
          (jit-lock-defer-timer (timer-create))
          (jit-lock--defer-timer-input-only t)
          (jit-lock-defer-buffers (list (current-buffer)))
          input-checks
      ...
      (cl-letf (((symbol-function 'input-pending-p)
                 (lambda (&optional _)
                   (setq input-checks (1+ (or input-checks 0)))
                   ;; The first call is the timer-entry guard.  The second
                   ;; happens after three regions have been scanned.
                   (> input-checks 1)))
                ((symbol-function 'redisplay)
                 (lambda (&optional _force)
                   (setq redisplayed t)
        ...
        (should (memq (current-buffer) jit-lock-defer-buffers))
        (should-not (eq (get-text-property 1 'fontified) 'defer))
        (should-not (eq (get-text-property 4 'fontified) 'defer))
        (should-not (eq (get-text-property 7 'fontified) 'defer))
        (should (eq (get-text-property 10 'fontified) 'defer))
        (should (= input-checks 2))
        (should (timerp jit-lock-defer-timer))))))
