...
struct emacs_metal_context
{
  ...
  dispatch_queue_t presenter_queue;
  CAMetalLayer *layer;
...
emacs_metal_context_create (void *view, int width, int height, int scale)
{
  ...
  ctx->presenter_queue =
    dispatch_queue_create (emacs_metal_presenter_queue_label,
                           DISPATCH_QUEUE_SERIAL);
  if (!ctx->presenter_queue)
    {
      emacs_metal_context_release (ctx);
      return NULL;
    }
...
{
  dispatch_async (ctx->presenter_queue, ^{
    @autoreleasepool
      {
                    ...
                    int dx, int dy)
{
  ...
  /* Metal does not guarantee useful results for overlapping same-texture
     blits.  Always stage scroll copies through a separate texture so scroll
     preservation behaves like the Core Graphics backing-store path.  */
