			 ...
			 ptrdiff_t end, ptrdiff_t end_byte, bool props)
{
  ...
  if (current_buffer->text->using_piece_table)
    {
      /* Piece table: text is not contiguous, use pt_get_text_emacs.  */
      pt_get_text_emacs (start_byte, end_byte - start_byte, (char *) SDATA (result));
    }
  else
#endif
    {
      /* Gap buffer: handle the gap by copying two regions if needed.  */
