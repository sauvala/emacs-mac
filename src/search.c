	      ...
	      ptrdiff_t *bytepos, bool allow_quit)
{
  ...
  if (count > 0)
	...
	/* BYTE_POS_ADDR and BUFFER_CEILING_OF handle both gap buffer
	   and piece table buffers, so memchr works for both.  */
        {
	...
	/* BYTE_POS_ADDR and BUFFER_FLOOR_OF handle both gap buffer
	   and piece table buffers, so memrchr works for both.  */
        {
          /* The termination address of the dumb loop.  */
