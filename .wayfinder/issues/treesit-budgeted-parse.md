...
- [Stage 2: cap runaway parses](27-stage2-budget-idle-slices.md): any parse past `treesit-budget-parse-limit` (2 s) is halted and its parser parses an empty input until `treesit-budget-retry`, so no readers see errors; the old grammar's hang file opens in the limit with a message, confirmed live by the user; no measurable cost; the map ends here unless stage 2b's trigger fires ([resolution](../comments/stage2-budget-idle-slices/2026-09-26-resolution.md))
